document.addEventListener("DOMContentLoaded", () => {
    // Konfiguracja gier z cenami i wygranymi
    const games = {
        lotto: {
            name: "Lotto",
            mainNumbers: 6,
            ...
            prizes: {
                ...
                4: 200,
                5: 4500,
                6: 4000000 // przykładowa wartość jackpota
            },
            hitLabels: {
                2: "2 trafienia",
                3: "3 trafienia", 
        ...
        eurojackpot: {
            ...
            prizes: {
                ...
                '5+0': 200000,
                '5+1': 5000000,
                '5+2': 150000000 // przykładowa wartość jackpota
            },
            hitLabels: {
                '5+2': "5 liczb + 2 dodatkowe (JACKPOT)",
                '5+1': "5 liczb + 1 dodatkowa (II nagroda)",
                '5+0': "5 liczb (III nagroda)",
                '4+2': "4 liczby + 2 dodatkowe (IV nagroda)",
                '4+1': "4 liczby + 1 dodatkowa (V nagroda)",
                '4+0': "4 liczby (VI nagroda)",
                '3+2': "3 liczby + 2 dodatkowe",
                '3+1': "3 liczby + 1 dodatkowa",
                '3+0': "3 liczby",
                '2+2': "2 liczby + 2 dodatkowe",
                '2+1': "2 liczby + 1 dodatkowa",
                '1+2': "1 liczba + 2 dodatkowe"
            },
    ...
    function updateGameUI() {
        elements.gameTitle.textContent = `Symulator ${currentGame.name}`;
        elements.gameRules.textContent = currentGame.rules;
        elements.switchLotto.classList.toggle("active", currentGame === games.lotto);
        elements.switchEurojackpot.classList.toggle("active", currentGame === games.eurojackpot);
        elements.userNumbersInput.placeholder = currentGame.extraNumbers > 0 
            ? `np. 5, 12, 18, 24, 35, 2, 7` 
            : `np. 5, 12, 18, 24, 35, 42`;
    ...
    function generateSets() {
        ...
        for (let i = 0; i < numSets; i++) {
            ...
            addSet(`<strong>Zestaw ${i + 1}:</strong> ${formatNumbers(numbers)}`);
        }
    ...
    function addCustomSet() {
        ...
        const allNumbers = numbers.split(",").map(num => parseInt(num.trim())).filter(num => !isNaN(num));
        ...
        const formattedNumbers = {
            ...
            extra: extraNumbers.sort((a, b) => a - b)
        };

        addSet(`<strong>Twój zestaw:</strong> ${formatNumbers(formattedNumbers)}`);
        enableDrawButtons();
        elements.userNumbersInput.value = "";
    }
    ...
    async function simulateUntilJackpot() {
        ...
        if (currentGame === games.lotto) {
            ...
            const prizeTiers = ['5+1', '5+0', '4+2', '4+1', '4+0', '3+2', '3+1', '3+0', '2+2', '2+1', '1+2'];
            prizeTiers.forEach(tier => {
                results.otherWins[tier] = 0;
            });
        }
            ...
            for (const userNumbers of sets) {
                ...
                if (isJackpot(hits)) {
                    results.jackpot = {
                        ...
                        numbers: userNumbers,
                        years: results.totalDraws / (currentGame.drawsPerWeek * 52),
                        hits: hits
                    };
                    foundJackpot = true;
                    break; 
                ...
                if (currentGame === games.lotto) {
                    ...
                    const key = `${hits.main}+${hits.extra}`;
                    if (results.otherWins[key] !== undefined) {
                        results.otherWins[key]++;
                    }
                }
            }
    ...
    function generateRandomNumbers() {
        ...
        while (mainNumbers.length < currentGame.mainNumbers) {
            const num = Math.floor(Math.random() * currentGame.mainPool) + 1;
            if (!mainNumbers.includes(num)) mainNumbers.push(num);
        }
        mainNumbers.sort((a, b) => a - b);

        if (currentGame.extraNumbers > 0) {
            const extraNumbers = [];
            while (extraNumbers.length < currentGame.extraNumbers) {
                const num = Math.floor(Math.random() * currentGame.extraPool) + 1;
        ...
        return { main: mainNumbers, extra: [] };
    }

    function formatNumbers(numbers) {
        const mainStr = numbers.main.map(n => n.toString().padStart(2, '0')).join(", ");
        if (numbers.extra && numbers.extra.length > 0) {
            return `${mainStr} | ${numbers.extra.map(n => n.toString().padStart(2, '0')).join(", ")}`;
        }
        return mainStr;
    }
    ...
    function countHits(draw, userNumbers) {
        const mainHits = draw.main.filter(num => userNumbers.main.includes(num)).length;
        let extraHits = 0;
        
        if (currentGame.extraNumbers > 0) {
            extraHits = draw.extra.filter(num => userNumbers.extra.includes(num)).length;
        }
        
        return { main: mainHits, extra: extraHits };
    }

    function getHitLabel(hits) {
        if (currentGame === games.lotto) {
            return currentGame.hitLabels[hits.main] || "";
        } else {
            const key = `${hits.main}+${hits.extra}`;
            return currentGame.hitLabels[key] || `${hits.main} liczby + ${hits.extra} dodatkowe`;
        }
    }

    function isJackpot(hits) {
        return currentGame.jackpotCondition(hits);
    }
    ...
    async function runSimulation(sets, numDraws) {
        ...
        for (let i = 0; i < numDraws; i++) {
            ...
            for (const userNumbers of sets) {
                ...
                if (hitsCounts[hitKey] !== undefined) {
                    hitsCounts[hitKey]++;
                }
                ...
                if (isJackpot(hits)) {
                    jackpots.push({
                        drawCount: i + 1,
                        numbers: userNumbers,
                        years: (i + 1) / (currentGame.drawsPerWeek * 52),
                        hits: hits
                    });
                }
            }
    ...
    function showResults(results, numDraws, timeMs) {
        ...
        let html = `
            <div class="simulation-summary">
                <strong>Podsumowanie symulacji:</strong><br>
                Liczba losowań: ${formatNumber(numDraws)}<br>
                Liczba zestawów: ${setsCount}<br>
                Czas trwania: ~${years} lat (${currentGame.drawsPerWeek} los./tydz.)<br>
                Czas wykonania: ${timeSec} sekund<br><br>
                
                <strong>Koszty i wygrane:</strong><br>
                Koszt zakładów: ${formatNumber(totalCost)} zł<br>
                Suma wygranych: ${formatNumber(totalWinnings)} zł<br>
                Bilans: <span style="color: ${profit >= 0 ? 'green' : 'red'}; font-weight: bold;">
    ...
    function showJackpotResults(results, timeMs, setsCount) {
        const timeSec = (timeMs / 1000).toFixed(2);
        const years = (results.totalDraws / (currentGame.drawsPerWeek * 52)).toFixed(2);
        const profit = results.totalWinnings - results.totalCost;
        const roi = (profit / results.totalCost * 100).toFixed(2);

        let html = `
            <div class="simulation-summary">
                <strong>Podsumowanie symulacji:</strong><br>
                Liczba losowań: ${formatNumber(results.totalDraws)}<br>
                Liczba zestawów: ${setsCount}<br>
                Czas trwania: ~${years} lat (${currentGame.drawsPerWeek} los./tydz.)<br>
                Czas wykonania: ${timeSec} sekund<br><br>
                
                <strong>Koszty i wygrane:</strong><br>
                Koszt zakładów: ${formatNumber(results.totalCost)} zł<br>
                Suma wygranych: ${formatNumber(results.totalWinnings)} zł<br>
                Bilans: <span style="color: ${profit >= 0 ? 'green' : 'red'}; font-weight: bold;">
                    ${profit >= 0 ? '+' : ''}${formatNumber(profit)} zł (${roi}%)
                </span>
            </div>
            <div style="margin-top: 20px;"><strong class="highlight">ZNALEZIONA GŁÓWNA WYGRANA:</strong></div>
    ...
    function handleYearsChange() {
        if (elements.drawYearsSelect.value !== "custom") {
            const years = parseInt(elements.drawYearsSelect.value);
            const drawsPerYear = currentGame.drawsPerWeek * 52;
            elements.numDrawsInput.value = years * drawsPerYear;
        }
    }

    function handleDrawsInput() {

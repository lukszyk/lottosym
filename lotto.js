document.addEventListener("DOMContentLoaded", () => {
    ...
    async function simulateUntilJackpot() {
        ...
        while (!foundJackpot) {
            ...
            if (results.totalDraws % 1000 === 0) { await new Promise(resolve => setTimeout(resolve, 0)); }
        }
    ...
    function showResults(results, numDraws, timeMs) {
        ...
        if (currentGame === games.lotto) { for (let i = 2; i <= 6; i++) { const count = results.hitsCounts[i] || 0; if (count > 0) { const ratio = (numDraws / count).toFixed(2); const prizeInfo = i >= 3 ? ` (${currentGame.prizes[i]} zł)` : ''; html += `<div style="margin: 5px 0;">${currentGame.hitLabels[i]}${prizeInfo}: <span class="hit-count">${formatNumber(count)}</span> (średnio 1 na ${ratio} losowań)</div>`; } } } else { const prizeTiers = ['5+2', '5+1', '5+0', '4+2', '4+1', '4+0', '3+2', '3+1', '3+0', '2+2', '2+1', '1+2']; prizeTiers.forEach(tier => { const count = results.hitsCounts[tier] || 0; if (count > 0 && currentGame.prizes[tier]) { const ratio = (numDraws / count).toFixed(2); const prizeInfo = ` (${currentGame.prizes[tier]} zł)`; html += `<div style="margin: 5px 0;">${currentGame.hitLabels[tier]}${prizeInfo}: <span class="hit-count">${formatNumber(count)}</span> (średnio 1 na ${ratio} losowań)</div>`; } }); }
        
        // --- SEKCJA Z NOWYM KODEM ---
    ...
    function showJackpotResults(results, timeMs, setsCount) { const timeSec = (timeMs / 1000).toFixed(2); const years = (results.totalDraws / (currentGame.drawsPerWeek * 52)).toFixed(2); const profit = results.totalWinnings - results.totalCost; const roi = (profit / results.totalCost * 100).toFixed(2); let html = `<div class="simulation-summary"><strong>Podsumowanie symulacji:</strong><br>Liczba losowań: ${formatNumber(results.totalDraws)}<br>Liczba zestawów: ${setsCount}<br>Czas trwania: ~${years} lat (${currentGame.drawsPerWeek} los./tydz.)<br>Czas wykonania: ${timeSec} sekund<br><br><strong>Koszty i wygrane:</strong><br>Koszt zakładów: ${formatNumber(results.totalCost)} zł<br>Suma wygranych: ${formatNumber(results.totalWinnings)} zł<br>Bilans: <span style="color: ${profit >= 0 ? 'green' : 'red'}; font-weight: bold;">${profit >= 0 ? '+' : ''}${formatNumber(profit)} zł (${roi}%)</span></div><div style="margin-top: 20px;"><strong class="highlight">ZNALEZIONA GŁÓWNA WYGRANA:</strong></div><div style="margin-top: 10px;"><span class="highlight">${getHitLabel(results.jackpot.hits)}</span> po ${formatNumber(results.jackpot.drawCount)} losowaniach (${results.jackpot.years.toFixed(2)} lat): <span class="numbers">${formatNumbers(results.jackpot.numbers)}</span></div>`; if (Object.values(results.otherWins).some(count => count > 0)) { html += `<div style="margin-top: 20px;"><strong>Inne wygrane podczas symulacji:</strong></div>`; if (currentGame === games.lotto) { for (let i = 3; i <= 5; i++) { if (results.otherWins[i] > 0) { html += `<div>${currentGame.hitLabels[i]}: ${results.otherWins[i]}</div>`; } } } else { for (const [tier, count] of Object.entries(results.otherWins)) { if (count > 0 && currentGame.prizes[tier]) { html += `<div>${currentGame.hitLabels[tier]}: ${count}</div>`; } } } } elements.resultsContainer.innerHTML = html; showResultsView(); }

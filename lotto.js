document.addEventListener("DOMContentLoaded", () => {
    ...
    const games = {
        lotto: {
            ...
            hitLabels: { 2: "2 trafienia", 3: "3 trafienia", 4: "4 trafienia", 5: "5 trafień", 6: "JACKPOT" },
    ...
    async function simulateDraws() {
        ...
        const setElements = Array.from(elements.setsContainer.children).filter(el => el.textContent.includes(":"));
        if (setElements.length === 0) { alert("Najpierw wygeneruj zestawy liczb."); return; }

        const trackedSetsForSimulation = setElements.map(el => {
            ...
            return {
                label: label,
                numbers: { main: mainNumbers, extra: extraNumbers },
                winCount: 0 // Ten licznik będzie teraz zliczał tylko jackpoty
            };
        });
        ...
        const results = await runSimulation(trackedSetsForSimulation, numDraws); 
    ...
    // MODYFIKACJA: Zlicza tylko JACKPOTY dla każdego zestawu
    async function runSimulation(trackedSets, numDraws) {
        const hitsCounts = {};
        const jackpots = [];
        ...
        for (let i = 0; i < numDraws; i++) {
            ...
            for (const trackedSet of trackedSets) {
                const hits = countHits(draw, trackedSet.numbers);
                ...
                // ZMIANA: Zliczamy wygrane tylko jeśli to jackpot
                if (isJackpot(hits)) { 
                    trackedSet.winCount++; // Inkrementuj licznik jackpotów dla tego zestawu
                    jackpots.push({ drawCount: i + 1, numbers: trackedSet.numbers, years: (i + 1) / (currentGame.drawsPerWeek * 52), hits: hits }); 
                }
            }
        ...
        updateProgress(numDraws, numDraws);
        return { hitsCounts, jackpots, topWinningSets: trackedSets };
    }
    ...
    // MODYFIKACJA: Wyświetla ranking zestawów, które trafiły GŁÓWNĄ WYGRANĄ
    function showResults(results, numDraws, timeMs) {
        ...
        const setsCount = results.topWinningSets.length;
        ...
        if (currentGame === games.lotto) { 
            for (let i = 2; i <= 6; i++) { 
                const count = results.hitsCounts[i] || 0; 
                if (count > 0) { 
                    const ratio = (numDraws * setsCount / count).toFixed(2); 
                    const prizeInfo = i >= 3 ? ` (${formatNumber(currentGame.prizes[i])} zł)` : ''; 
                    html += `<div style="margin: 5px 0;">${currentGame.hitLabels[i]}${prizeInfo}: <span class="hit-count">${formatNumber(count)}</span> (średnio 1 na ${formatNumber(ratio)} zakładów)</div>`; 
                } 
            } 
        } else { 
            const prizeTiers = ['5+2', '5+1', '5+0', '4+2', '4+1', '4+0', '3+2', '3+1', '3+0', '2+2', '2+1', '1+2']; 
            prizeTiers.forEach(tier => { 
                ...
                if (count > 0 && currentGame.prizes[tier]) { 
                    const ratio = (numDraws * setsCount / count).toFixed(2); 
                    const prizeInfo = ` (${formatNumber(currentGame.prizes[tier])} zł)`; 
                    html += `<div style="margin: 5px 0;">${currentGame.hitLabels[tier]}${prizeInfo}: <span class="hit-count">${formatNumber(count)}</span> (średnio 1 na ${formatNumber(ratio)} zakładów)</div>`; 
                } 
            }); 
        }
        ...
        if (results.jackpots.length > 0) {
            ...
            if (yearDifferences.length > 0) {
                ...
                html += `<div style="margin-top: 15px; background-color: #f0f8ff; padding: 8px; border-radius: 3px;"><strong>Statystyki odstępów między głównymi wygranymi:</strong><br>Najkrótszy odstęp: ${minDiff} lat<br>Najdłuższy odstęp: ${maxDiff} lat<br>Średni odstęp: ${avgDiff} lat</div>`;
            }
        }
        
        const { topWinningSets } = results;
        const winningSetsSorted = topWinningSets
            .filter(set => set.winCount > 0)
            .sort((a, b) => b.winCount - a.winCount)
            .slice(0, 5);

        if (winningSetsSorted.length > 0) {
            // ZMIANA: Zmieniony nagłówek
            html += `<div style="margin-top: 20px;"><strong>Top 5 zestawów z główną wygraną:</strong></div>`;
            html += `<ul style="list-style-type: none; padding-left: 0;">`;
            winningSetsSorted.forEach((set, index) => {
                html += `
                    <li style="background-color: ${index % 2 === 0 ? '#f9f9f9' : '#fff'}; padding: 8px; border-radius: 4px; margin-bottom: 5px;">
                        <span style="font-weight: bold; color: #333;">${index + 1}. ${set.label}</span>
                        <span class="numbers">${formatNumbers(set.numbers)}</span>
                        - trafień głównych: <strong class="hit-count">${formatNumber(set.winCount)}</strong> razy
                    </li>
                `;
            });
            html += `</ul>`;
